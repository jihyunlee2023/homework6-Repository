...
int main(void)
{
   QueueType *cQ = createQueue(); //createQueue 함수를 호출하여 큐를 생성하고 큐를 가리키는 QueueType 구조체 포인터 cQ 선언
...
int freeQueue(QueueType *cQ)
{
    if(cQ == NULL) return 1; //큐가 NULL이면 1 반환
...
void deQueue(QueueType *cQ, element *item)
{
   if(isEmpty(cQ)) // 큐가 비어있으면 
      ...
      *item = cQ->queue[cQ->front]; //큐의 front 위치에 있는 데이터를 item이 가리키는 변수에 저장
   }
}
